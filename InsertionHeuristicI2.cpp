...
vector<CostMap> find_insertion(
    ...
    double alpha1, double alpha2, double beta1, double beta2, double vehicle_capacity, double m
) {
...
pair<vector<int>, vector<int>> i2_heuristic(
    double alpha1, double alpha2, double beta1, double beta2, double vehicle_capacity, double m,
    ...
    while (true) {
        ...
        vector<CostMap> position = find_insertion(route, unvisited, customers, distance, curr_route_time, alpha1, alpha2, beta1, beta2, vehicle_capacity, m);
        if (position.empty()) {
            break;
        }
        for (const auto& item: position) {
            for (const auto& [u, cost]: item) {
                ...
                c2_list.push_back({(double)u, beta1 * min_c1 + beta2 * calculate_new_time(curr_route_time, u, insertion, route[insertion - 1], route[insertion], customers, distance), (double)insertion});
            }
        }
        sort(c2_list.begin(), c2_list.end(), [](const vector<double>& a, const vector<double>& b) {
            return a[1] < b[1];
        });
        // for (int i = 0; i < c2_list.size(); i++) {
        //     cout << c2_list[i][0] << " " << c2_list[i][1] << " " << c2_list[i][2] << endl;
        // }
...
vector<vector<int>> insertion(
    ...
    vector<int> cluster;
    for (int i = 0; i < curr_customer.size(); i++) {
        cluster.push_back(curr_customer[i]);
    }
    ...
    for (const auto& cluster: clusters) {
        ...
        tie(route, remain_node) = i2_heuristic(alpha1, alpha2, beta1, beta2, vehicle_capacity, m, cluster, customers, distance);
        remain.insert(remain.end(), remain_node.begin(), remain_node.end());
        routes.push_back(route);
    }
    if (!remain.empty()) {
        vector<vector<int>> remain_routes = insertion(alpha1, alpha2, beta1, beta2, vehicle_capacity, m, remain, customers, distance);
...
int main() {
    if (fopen("C:\\Users\\Admin\\Desktop\\c++\\.vscode\\Project1\\input.txt", "r")) {
        ...
        freopen("C:\\Users\\Admin\\Desktop\\c++\\.vscode\\Project1\\outputIH_I2.txt", "w", stdout);
    }
    int vehicle_count;
    ...
    double beta1 = 0.5;
    double beta2 = 1 - beta1;
    vector<vector<int>> routes = insertion(alpha1, alpha2, beta1, beta2, vehicle_capacity, m, remain_customers, customers, distance);
