...
vector<vector<int>> insertion(
    ...
    if (!remain.empty()) {
        vector<vector<int>> remain_routes = insertion(alpha1, alpha2, lambda, vehicle_capacity, m, remain, customers, distance);
...
int main() {
    if (fopen("C:\\Users\\Admin\\Desktop\\c++\\.vscode\\Project1\\input.txt", "r")) {
        ...
        freopen("C:\\Users\\Admin\\Desktop\\c++\\.vscode\\Project1\\outputIH_I1.txt", "w", stdout);
    }
    int vehicle_count;
    ...
    double alpha1 = 1;
    double alpha2 = 0;
    vector<vector<int>> routes = insertion(alpha1, alpha2, lambda, vehicle_capacity, m, remain_customers, customers, distance);
